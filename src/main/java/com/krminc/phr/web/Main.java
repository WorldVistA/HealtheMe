...
public class Main {
    ...
    public Long getAuthenticatedUserId() {
        if (userId == null) {
            ...
            EntityManager em = persistenceSvc.getEntityManager();
            try {
                userId = (Long) em.createNamedQuery("User.findUserIdByUsername").setParameter("username", getUsername()).getSingleResult();
            } catch (Exception e) {
...
//        logger.error("Asked for health record Id.");
    ...
    public Long getPrimaryHealthRecordId(Long userId) {
        if (healthRecordId == null || healthRecordId < 0) {
            ...
            try {
                ...
                healthRecordId = user.getPrimaryHealthRecord().getHealthRecordId();
            } catch (Exception e) {
    ...
    private boolean setLoginUpdates(Long loggedInUserId) {
        ...
        if (loggedInUserId > 0) {
            ...
            try {
                ...
                if (em.createNamedQuery("User.updateLoggedInTime").setParameter("userId", loggedInUserId).executeUpdate() == 1) {
                    if (em.createNamedQuery("User.updateLoggedInTotal").setParameter("userId", loggedInUserId).executeUpdate() == 1) {
                        returnValue = true;
                    }
                }
    ...
    private UserPreferences getPreferencesByRecId(Long recId) {
        ...
        try {
            HealthRecord hr = (HealthRecord) em.createNamedQuery("HealthRecord.findByHealthRecordId")
                .setParameter("healthRecordId", recId)
                .getSingleResult();
            
            pref = hr.getPreferences();
        } catch (Exception e) {
            // logger.warn("Unable to retrieve UserPreferences for user.", e); // common case, not worth warning
            pref = null;
        } finally {
            persistenceSvc.close();
        }
     ...
     *
     * <p>Performs a patient authorization check for all method calls that pass through here.</p>
     *
     * @param patientId
     * @param resourceType
     * @param resourceKey
     ...
     */
    private Object patientWebResource(
        Long healthRecordId, String resourceType, String resourceKey, Long resourceId
    ) {
        ...
        try {
            persistenceSvc.beginTx();
            return em.createNamedQuery(resourceType + ".findByPrimaryKeyForRecord")
                .setParameter("healthRecordId", healthRecordId)
                .setParameter(resourceKey, resourceId)
                .getSingleResult();
     ...
     *  This differs from the patientWebResource method in that this is a 1-per-patient mapping which
     *  is mapped on health record id rather than the id of the object
     *  Note that this method uses a different namedquery and handles errors differently
     *
     * <p>Performs a patient authorization check for all method calls that pass through here.</p>
     *
     * @param patientId
     ...
     * @return Object of the resourceType
     */
    private Object patientCareResource(
        Long healthRecordId, String qualifiedPackageType, String resourceType
    ) {
        authorizeFor(healthRecordId);
...
//            logger.error("Unable to find a result for NamedQuery {}.findByHealthRecordId", resourceType);
            ...
            try {
                ...
                for (Constructor c : constructors) {
                    ...
                    if (types.length > 0 ) {
                        for (Class type : types) {
                            if (type.getName().equalsIgnoreCase(Long.class.getName())){
                                try {
                ...
                //fall back on default constructor if cant call by type for plugging in hrid
    ...
    private Collection<Object> patientCareResources(
        Long healthRecordId, String qualifiedPackageType, String resourceType
    ) {
        authorizeFor(healthRecordId);
...
//            logger.error("Unable to find a result for NamedQuery {}.findByHealthRecordId", resourceType);
                        ...
                        for (Class type : types) {
                            if (type.getName().equalsIgnoreCase(Long.class.getName())){
                                try {
                                    ...
                                    singleEntryList.add(c.newInstance(healthRecordId));
    ...
    public PHRCalendar getCalendar(
            @PathParam("patientId") Long patientId) {
        authorizeFor(patientId);
        PHRCalendar calendar = new PHRCalendar(patientId);
        // TODO: we probably need to add something to deal with the date range (DAY, WEEK, MONTH, YEAR)
        return calendar;
    }

    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/calendar/{year: (19|20)\\d{2}}}/")
    public PHRCalendar getCalendar(
            @PathParam("patientId") Long patientId,
    ...
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/calendar/{year: (19|20)\\d{2}}/{month: (0[1-9]|1[012]|[1-9])}/")
    public PHRCalendar getCalendar(
            @PathParam("patientId") Long patientId,
            @PathParam("year") int year,
    ...
    public Allergy getAllergy(
        ...
        return (Allergy) patientWebResource(patientId, "Allergy", "allergyId", allergyId);
    }
     ...
     * @return Immunization
     */
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/immunizations/{immunizationId}/")
    public Immunization getImmunization(
            @PathParam("patientId") Long patientId,
            @PathParam("immunizationId") Long immunizationId) {
        return (Immunization) patientWebResource(patientId, "Immunization", "immunizationId", immunizationId);
    }
     ...
     * @return Medication
     */
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/medications/{medicationId}/")
    public Medication getMedication(
            @PathParam("patientId") Long patientId,
            @PathParam("medicationId") Long medicationId) {
        return (Medication) patientWebResource(patientId, "Medication", "medicationId", medicationId);
    }
     ...
     * @return Problem
     */
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/medicalevents/{medicalEventId}/")
    public MedicalEvent getMedicalEvent(
            @PathParam("patientId") Long patientId,
            @PathParam("medicalEventId") Long medicalEventId) {
        return (MedicalEvent) patientWebResource(patientId, "MedicalEvent", "medicalEventId", medicalEventId);
    }
    ...
    public Visit getVisit(
        ...
        return (Visit) patientWebResource(patientId, "Visit", "visitId", visitId);
    }
    ...
    public Exercise getExercise(
        ...
        return (Exercise) patientWebResource(patientId, "Exercise", "exerciseId", exerciseId);
    }

    //vitals paths
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/vitals/bloodpressure/{bloodpressureid}/")
    public BloodPressure getBloodPressure(
            @PathParam("patientId") Long patientId,
            @PathParam("bloodpressureid") Long bloodPressureId) {
        return (BloodPressure) patientWebResource(patientId, "BloodPressure", "bloodPressureId", bloodPressureId);
    }

    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/vitals/bloodsugar/{bloodsugarid}/")
    public BloodSugar getBloodSugar(
            @PathParam("patientId") Long patientId,
            @PathParam("bloodsugarid") Long bloodSugarId) {
        return (BloodSugar) patientWebResource(patientId, "BloodSugar", "bloodSugarId", bloodSugarId);
    }

    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/vitals/temperature/{temperatureid}/")
    public Temperature getTemperature(
            @PathParam("patientId") Long patientId,
            @PathParam("temperatureid") Long temperatureId) {
        return (Temperature) patientWebResource(patientId, "Temperature", "temperatureId", temperatureId);
    }

    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/vitals/weight/{weightid}/")
    public Weight getWeight(
            @PathParam("patientId") Long patientId,
            @PathParam("weightid") Long weightId) {
        return (Weight) patientWebResource(patientId, "Weight", "weightId", weightId);
    }

    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/vitals/heartrate/{heartrateid}/")
    public HeartRate getHeartRate(
            @PathParam("patientId") Long patientId,
            @PathParam("heartrateid") Long heartRateId) {
        return (HeartRate) patientWebResource(patientId, "HeartRate", "heartRateId", heartRateId);
    }

    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/vitals/height/{heightid}/")
    public Height getHeight(
            @PathParam("patientId") Long patientId,
            @PathParam("heightid") Long heightId) {
        return (Height) patientWebResource(patientId, "Height", "heightId", heightId);
    }
    ...
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/vitals/peakflow/{peakflowid}/")
    public PeakFlow getPeakFlow(
            @PathParam("patientId") Long patientId,
            @PathParam("peakflowid") Long peakFlowId) {
        return (PeakFlow) patientWebResource(patientId, "PeakFlow", "peakFlowId", peakFlowId);
    }
    ...
    public EmergencyInformation getEmergencyInformation(
        ...
        return (EmergencyInformation) patientCareResource(patientId, "com.krminc.phr.domain.carenotebook", "EmergencyInformation");
    }
    ...
    public InsurancesInformation getInsuranceInformation(
        ...
        Vector<InsuranceInformation> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "InsuranceInformation");
    ...
    public SpecialtyClinics getSpecialtyClinics(
        ...
        Vector<SpecialtyClinic> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "SpecialtyClinic");
    ...
    public Pharmacies getPharmacies(
        ...
        Vector<Pharmacy> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Pharmacy");
    ...
    public CareProviders getCareProviders(
        ...
        Vector<CareProvider> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "CareProvider");
    ...
    public FamilyMembers getFamilyMembers(
        ...
        Vector<FamilyMember> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "FamilyMember");
    ...
    public FamilyHistory getFamilyHistory(
        ...
        return (FamilyHistory) patientCareResource(patientId, "com.krminc.phr.domain.carenotebook", "FamilyHistory");
    }
    ...
    public Nutrition getNutrition(
        ...
        return (Nutrition) patientCareResource(patientId, "com.krminc.phr.domain.carenotebook", "Nutrition");
    }
    ...
    public RespiratoryCareLog getRespiratoryCareLog(
        ...
        Vector<RespiratoryCare> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "RespiratoryCare");
    ...
    public CommunicationLog getCommunicationLog(
        ...
        Vector<Communication> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Communication");
        return new CommunicationLog(c, patientId);
    }
    
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/mobility/")
    public MobilityLog getMobilityLog(
        @PathParam("patientId")  Long patientId
    ) {
        Vector<Mobility> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Mobility");
    ...
    public RestLog getRestLog(
        ...
        Vector<Rest> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Rest");
        return new RestLog(c, patientId);
    }
    
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/social/")
    public SocialLog getSocialLog(
        @PathParam("patientId")  Long patientId
    ) {
        Vector<Social> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Social");
        return new SocialLog(c, patientId);
    }
    
    @Path(AppConfig.PATH_PATIENT_ROOT + "/{patientId}/stress/")
    public StressLog getStressLog(
        @PathParam("patientId")  Long patientId
    ) {
        Vector<Stress> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Stress");
    ...
    public Transition getTransitions(
        ...
        return (Transition) patientCareResource(patientId, "com.krminc.phr.domain.carenotebook", "Transition");
    }
    ...
    public Employments getEmployments(
        ...
        Vector<Employment> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Employment");
    ...
    public Schools getSchools(
        ...
        Vector<School> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "School");
    ...
    public SchoolPersonnels getSchoolPersonnel(
        ...
        Vector<SchoolPersonnel> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "SchoolPersonnel");
    ...
    public Meetings getMeetings(
        ...
        Vector<Meeting> c = (Vector) patientCareResources(patientId, "com.krminc.phr.domain.carenotebook", "Meeting");
