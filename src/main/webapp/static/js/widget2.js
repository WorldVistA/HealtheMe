...
function updateSelfEnteredJSON(obj,id) {
    for (var key in obj) {
        if(typeof obj[key] != 'object') {
            switch(key){
                case "dateAdded":
                    $("tr#" +id).next("tr span[class='" + key + "']").text(obj[key]);
                    break;
                default:
                    $("tr#" +id+" span[class='" + key + "']").text(obj[key]);
                    break;
            }
        }
    }
...
function renderSelfEnteredJSON(obj) {
    for (var key in obj) {
        if(typeof obj[key] == 'object') {
            if (obj[key].id != null) {
                var row = $("tr.rowtemplate.selfentered").clone().removeClass().addClass("self resultrow row" + key).show().attr("id",obj[key].id);
                var row2 = $("tr.detailtemplate.selfentered").clone().removeClass().addClass("self resultdetailrow row" + key + "detail ui-helper-hidden");
                $("tr.rowtemplate").parent().append(row).append(row2);
                $("tr:last span[class='source']").text("Self Entered");
            }
            renderSelfEnteredJSON(obj[key]);
        }
        else {
            switch(key){
                case "dateAdded":
...
function renderClinicalJSON(obj) {
    for (var key in obj) {
        if(typeof obj[key] == 'object') {
            if (obj[key].id != null) {
                var row = $("tr.rowtemplate.clinical").clone().removeClass().addClass("clinical resultrow row" + key).show().attr("id",obj[key].id);
                var row2 = $("tr.detailtemplate.clinical").clone().removeClass().addClass("clinical resultdetailrow row" + key + "detail ui-helper-hidden");
                $("tr.rowtemplate").parent().append(row).append(row2);
        ...
        success:function(data){
            ...
            if(containsAnObject(data)) {
                if (source=="clinical") {
                    ...
                    $("tr.rowtemplate").parent().append($("tr.titletemplate.selfentered").clone().removeClass().addClass("titlerow").show());
                    renderSelfEnteredJSON(data);
                }
            }
